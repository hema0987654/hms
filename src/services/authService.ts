...
import { insertTempUser, findTempUserByEmailAndOtp, deleteTempUser } from "../models/tempUser.js";
...
async SignUp(infoUsers: Info) {
    ...
    const passwordHash = await bycrbt.hash(infoUsers.password, 10);

    const otp = otpGenerator.generate(6, { upperCaseAlphabets: false, specialChars: false, lowerCaseAlphabets: false });
    const otpExpires = new Date(Date.now() + 5*60*1000);

    await insertTempUser({ ...infoUsers, password: passwordHash, otp, otpExpires });

    await sendOtpEmail(infoUsers.email,otp);

return { success: true, message: "OTP has been sent to your email to verify your account" };
  }
...
return { success: true, message: "Account has been successfully activated", token, user: [newUser] };
  }
 ...
 async loginWithGoogle(code: string) {
  try {
    ...
    if (!payload) return { success: false, message: "Google authentication failed" };
